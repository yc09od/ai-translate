...
feature：/topics/:topicId/translation/live，原本保存每段句子为文件，现在添加新功能，不仅仅保存句子为文件，同时异步的发送这段二进制音频给ai api。使用我们的pre-prompt要求ai api翻译。注意，保存句子和发送翻译请求应该同时进行。所有进程可以不等待文件保存。
